...
select  
	f.flig_fly_to as fly_to,
	f.flig_city_to as city_to,
  	avg(f.flig_price * sp.sear_currency_rate) as price
from flight as f, search_param as sp 
where f.sear_id = sp.sear_id
group by f.flig_fly_to, f.flig_city_to
;
...
select  
	...
	date(f.flig_dtime),
	date(f.flig_atime),
  avg(f.flig_price * sp.sear_currency_rate)
from flight as f, search_param as sp 
where f.sear_id = sp.sear_id
group by 
	f.flig_fly_to,
...
select  
	f.flig_fly_to as fly_to,
		fbp.flbp_hand as hand,
	  	avg(fbp.flbp_price * sp.sear_currency_rate) as price
from flight as f, search_param as sp, flight_bags_price as fbp
where f.sear_id = sp.sear_id
  and f.flig_id = fbp.flig_id
  and sp.sear_id = 2
  and f.flig_fly_to = 'LIS'
group by 
	f.flig_fly_to,
	fbp.flbp_hand
;
...
delete from flight_bags_price;
delete from flight_conversion;
delete from flight;
delete from search_param;


drop table search_param cascade;
drop table flight_airlines cascade;
drop table flight_bags_price cascade;
drop table flight_conversion cascade;
